...
[![Live Demo](https://img.shields.io/badge/⚔️%20Live%20Arena-HF%20Space-orange?style=for-the-badge)](https://huggingface.co/spaces/ayussssssiiii/codecourt)
...
[![Colab](https://img.shields.io/badge/📓%20Training-Colab%20Notebook-yellow?style=for-the-badge)](YOUR_COLAB_LINK)
...
| 📝 **Mini Blog (HF)** | [Read on HuggingFace](https://huggingface.co/blog/ayussssssiiii/codecourt) |
...
Every LLM coding benchmark shares a fatal flaw: **the model may have seen those problems during pretraining.** High pass rates don't prove robustness — they prove memorization.

Real software fails on things nobody thought to test:

- `n=0`, `n=MAX_INT`, empty graphs, single-node trees
- Off-by-one traps planted by adversaries  
- Complexity bombs — O(n²) that passes small inputs but TLEs on stress tests
- Code that looks correct but breaks under one specific adversarial input
...
| Agent | Role | Goal |
|---|---|---|
| **Setter** | Red Team | Plant hidden edge cases, complexity traps, zero-day bugs |
| **Solver** | Blue Team | Pass ALL tests including adversarial hidden ones |
| **Oracle** | Sandboxed Judge | Run real code, measure pass/fail, issue shaped rewards |

The Setter is rewarded **exactly when the Solver fails.** This creates a genuine co-evolving adversarial loop — not a static benchmark. The Solver cannot memorize its way to high rewards.
...
*The spike to +34.31 at step 34 is the key signal: when the model generates a complete solution within the token budget, it passes adversarial hidden tests. The reward design is correct — the bottleneck is generation length, not learning ability.*
...
**3. Multi-signal shaped reward** — 5 independent checks. Gaming one doesn't save you on the others:
...
**4. Setter actively adapts** — the Red Team is rewarded when it finds weaknesses. It doesn't stand still.
...
└── outputs/
    ├── training_history.json        # 101 episodes
...
**[Live Demo](https://huggingface.co/spaces/ayussssssiiii/codecourt) · [Trained Model](https://huggingface.co/ayussssssiiii/codecourt-solver-grpo-v1) · [GitHub](https://github.com/ayushoncode/CodeCourt) · [Blog](https://huggingface.co/blog/ayussssssiiii/codecourt)**
